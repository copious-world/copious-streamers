...
function rtrim_char(char,str) {
    ...
    if ( str[str.length-1] === char ) return(str.substring(0,(str.length-1)))
    return str
}
...
class ConnectedCounter extends PlayCounter {
    ...
    constructor(net_conf,app_link_manager,crypto_conf,info_path,update_interval) {
        super(net_conf,link_manager_conf,crypto_conf,info_path,update_interval)
        ...
        //
        if ( app_link_manager ) {
            this._link_manager = app_link_manager
    ...
    async add_relay_path(persistence_link) {
        ...
        let link_request_handler = async (msg) => {         // link_request_handler
            ...
            let asset = `${msg.protocol}/${tracking}`   // actually the clear cwid
    ...
    async send_link(mobj,path) {
        ...
        //
        let link = `${this.url_prefix}/${mobj.protoco}/${clear_cwid}`
        mobj.resolver = () => {}
        let res_obj = {
            "_id" : this.service_id,
            "streamer_link" : link,
        ...
        let response = await this.relayer.send_op_on_path(res_obj,path,"RSP")
